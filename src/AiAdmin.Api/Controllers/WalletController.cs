...
public sealed class WalletController(AppDbContext db) : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<WalletResult>>> GetCurrentAsync() {
        ...
        return wallet is null ? NotFound(new ApiResponse<object>(404, "Wallet not found", null)) : Ok(ApiResponse<WalletResult>.Ok(ToResult(wallet)));
    }
    ...
    private static WalletListItem ToListItem(Wallet wallet) {
        return new WalletListItem(
            wallet.UserId, "USD", wallet.AvailableBalance, wallet.FrozenBalance, wallet.TotalIncome, wallet.TotalExpense
            , wallet.LastTransactionAt.HasValue ? new DateTimeOffset(wallet.LastTransactionAt.Value) : null, wallet.Version, wallet.UserId
            , new DateTimeOffset(wallet.CreatedAt), wallet.User.UserName, wallet.User.Email, wallet.User.Avatar
        );
    }
    ...
    private static WalletResult ToResult(Wallet wallet) {
        return new WalletResult(
            wallet.UserId, "USD", wallet.AvailableBalance, wallet.FrozenBalance, wallet.TotalIncome, wallet.TotalExpense
            , wallet.LastTransactionAt.HasValue ? new DateTimeOffset(wallet.LastTransactionAt.Value) : null, wallet.Version
        );
    }
}