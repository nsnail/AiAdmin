...
public static class ApiMessages
{
    ...
    private static readonly Dictionary<string, (string Zh, string En)> _messages = new()
    {
        ...
        , ["loginProofInvalid"] = ("登录验证已过期，请重新验证", "Login verification expired, please try again")
        , ["registrationDisabled"] = ("当前未开启用户注册", "User registration is disabled")
        , ["emailExists"] = ("邮箱已存在", "Email already exists")
        , ["registerSuccess"] = ("注册成功", "Registration successful")
        ...
        , ["departmentCodeExists"] = ("部门编码已存在", "Department code already exists")
        , ["departmentParentNotFound"] = ("父部门不存在", "Parent department not found")
        , ["departmentCreated"] = ("部门创建成功", "Department created")
        , ["departmentNotFound"] = ("部门不存在", "Department not found")
        , ["departmentHasChildren"] = ("请先删除子部门", "Delete child departments first")
        , ["departmentHasUsers"] = ("部门已关联用户，无法删除", "The department has assigned users")
        , ["departmentDeleted"] = ("部门删除成功", "Department deleted")
        , ["departmentCycle"] = ("父部门不能是当前部门或其子部门", "Parent department cannot be itself or its descendant")
        , ["departmentUpdated"] = ("部门更新成功", "Department updated")
        , ["invalidDepartment"] = ("包含无效部门", "One or more departments are invalid")
