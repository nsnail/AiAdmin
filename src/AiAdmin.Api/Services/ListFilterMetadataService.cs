...
public static class ListFilterMetadataService
{
    ...
    public static IReadOnlyList<ListFilterFieldResult> GetFields<TEntity>()
    {
        return typeof(TEntity).GetProperties(BindingFlags.Instance | BindingFlags.Public)
            .Select((property, index) => (Property: property, Attribute: property.GetCustomAttribute<ListFilterAttribute>(), DeclarationIndex: index))
            ...
            .Select(item => new ListFilterFieldResult(
                ...
                item.Attribute.Options.Select(ToOption).ToArray(),
                GetValueType(item.Property.PropertyType)
            )).ToArray();
    }
    ...
    private static ListFilterOptionResult ToOption(string option)
    {
        ...
        return separator < 0 ? new ListFilterOptionResult(option, option) : new ListFilterOptionResult(option[(separator + 1)..], option[..separator]);
    }
