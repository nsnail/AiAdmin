...
async function showCleanupWarning() {
    createCard('安全警告', [`${fmt.warning('此操作将永久删除以下演示内容，且无法恢复！')}`, `${fmt.dim('请仔细阅读清理列表，确认后再继续操作')}`])
...
async function showStats() {
    ...
    createCard('清理统计', [
        ...
        ...(stats.failedPaths > 0 ? [`${icons.error} ${fmt.error('删除失败')}: ${fmt.highlight(stats.failedPaths.toString())} 个路径`] : []),
