<template>
  <ElInput v-model="textValue" v-bind="$attrs" class="sc-cron-input">
    <template #append>
      <ElDropdown trigger="click" @command="handleCommand">
        <ElButton aria-label="打开 Cron 快捷规则" :icon="ArrowDown" />
        <template #dropdown>
          <ElDropdownMenu>
            <ElDropdownItem v-for="item in shortcuts" :key="item.value" :command="item.value">
              {{ item.label }}
            </ElDropdownItem>
            <ElDropdownItem divided command="custom">{{ t('cronEditor.custom') }}</ElDropdownItem>
          </ElDropdownMenu>
        </template>
      </ElDropdown>
    </template>
  ...
  <ElDialog v-model="editorVisible" :title="t('cronEditor.title')" width="760px" destroy-on-close append-to-body>
      ...
      <ElTabPane v-for="field in fields" :key="field.key" :name="field.key">
        ...
        <ElForm label-width="70px">
          ...
          <ElFormItem v-if="state[field.key].type === 'range'" :label="t('cronEditor.range')">
            <ElInputNumber v-model="state[field.key].start" :min="field.min" :max="field.max" controls-position="right" />
            <span class="sc-cron-separator">{{ t('cronEditor.to') }}</span>
            <ElInputNumber v-model="state[field.key].end" :min="field.min" :max="field.max" controls-position="right" />
          </ElFormItem>
          <ElFormItem v-if="state[field.key].type === 'step'" :label="t('cronEditor.step')">
            <ElInputNumber v-model="state[field.key].start" :min="field.min" :max="field.max" controls-position="right" />
            <span class="sc-cron-separator">{{ t('cronEditor.every') }}</span>
            <ElInputNumber v-model="state[field.key].step" :min="1" :max="field.max - field.min + 1" controls-position="right" />
            <span class="sc-cron-unit">{{ field.unit }}</span>
          </ElFormItem>
          <ElFormItem v-if="state[field.key].type === 'list'" :label="t('cronEditor.list')">
            <ElSelect v-model="state[field.key].list" multiple filterable class="w-full">
    ...
    <template #footer>
      ...
      <ElButton type="primary" @click="apply">{{ t('common.confirm') }}</ElButton>
    </template>
  </ElDialog>
</template>
...
<script setup lang="ts">
  ...
  type Field = { key: FieldKey; label: string; unit: string; min: number; max: number; options: number[] }
  const { t } = useI18n()

  const props = withDefaults(defineProps<{ modelValue?: string }>(), { modelValue: '0 */5 * * * *' })
  const emit = defineEmits<{ (event: 'update:modelValue', value: string): void }>()
  const fields = computed<Field[]>(() => [
    { key: 'second', label: t('cronEditor.fields.second'), unit: t('cronEditor.units.second'), min: 0, max: 59, options: Array.from({ length: 60 }, (_, i) => i) },
    { key: 'minute', label: t('cronEditor.fields.minute'), unit: t('cronEditor.units.minute'), min: 0, max: 59, options: Array.from({ length: 60 }, (_, i) => i) },
    { key: 'hour', label: t('cronEditor.fields.hour'), unit: t('cronEditor.units.hour'), min: 0, max: 23, options: Array.from({ length: 24 }, (_, i) => i) },
    { key: 'day', label: t('cronEditor.fields.day'), unit: t('cronEditor.units.day'), min: 1, max: 31, options: Array.from({ length: 31 }, (_, i) => i + 1) },
    { key: 'month', label: t('cronEditor.fields.month'), unit: t('cronEditor.units.month'), min: 1, max: 12, options: Array.from({ length: 12 }, (_, i) => i + 1) },
    { key: 'week', label: t('cronEditor.fields.week'), unit: t('cronEditor.units.week'), min: 0, max: 6, options: Array.from({ length: 7 }, (_, i) => i) }
  ])
  ...
  const state = reactive<Record<FieldKey, RuleState>>({
    second: createRule(), minute: createRule(), hour: createRule(), day: createRule(), month: createRule(), week: createRule()
  })
