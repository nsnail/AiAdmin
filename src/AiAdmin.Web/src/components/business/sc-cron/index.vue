...
<script setup lang="ts">
  ...
  const generatedValue = computed(() =>
    fields.value.map((field) => expressionFor(field.key)).join(' ')
  )
  ...
  const cronDescription = computed(() => {
    ...
    if (time && day === '*' && month === '*' && week === '*')
      return t('cronEditor.description.daily', { time })
    if (time && day === '*' && month === '*' && /^\d$/.test(week))
      return t(`cronEditor.description.weekly${week}`, { time })
