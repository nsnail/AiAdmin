<!-- 基础横幅组件 -->
<template>
    <div :class="[{ 'has-decoration': decoration }, boxStyle]" :style="{ height }" @click="emit('click')" class="art-card basic-banner">
        <!-- 流星效果 -->
        ...
        <div class="basic-banner__content">
            <!-- title slot -->
            <slot name="title">
                <p v-if="title" :style="{ color: titleColor }" class="basic-banner__title">{{ title }}</p>
            </slot>

            <!-- subtitle slot -->
            <slot name="subtitle">
                <p v-if="subtitle" :style="{ color: subtitleColor }" class="basic-banner__subtitle">{{ subtitle }}</p>
            </slot>
