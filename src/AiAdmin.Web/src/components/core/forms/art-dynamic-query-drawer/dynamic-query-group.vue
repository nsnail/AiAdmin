  ...
  <div class="query-group">
    ...
    <div v-for="(item, index) in model.filters" :key="item.id" class="query-node">
      ...
      <div v-else class="condition-row">
        ...
        <ElSelect v-if="selectedField(item.field)?.type === 'boolean'" v-model="item.value" class="value-input" placeholder="值">
        ...
        <ElButton circle text type="danger" aria-label="删除条件" @click="model.filters.splice(index, 1)">
...
<script setup lang="ts">
  ...
  const operators = [
    ...
    { label: '等于', value: 'Equal' }, { label: '不等于', value: 'NotEqual' },
    { label: '大于', value: 'GreaterThan' }, { label: '大于等于', value: 'GreaterThanOrEqual' },
    { label: '小于', value: 'LessThan' }, { label: '小于等于', value: 'LessThanOrEqual' },
    { label: '范围', value: 'Range' }, { label: '日期范围', value: 'DateRange' },
    { label: '任一匹配', value: 'Any' }, { label: '均不匹配', value: 'NotAny' },
  ...
  const addCondition = () => model.value.filters.push({ id: crypto.randomUUID(), kind: 'condition', field: '', operator: 'Contains', value: '' })
  const addGroup = () => model.value.filters.push({ id: crypto.randomUUID(), kind: 'group', group: { logic: 'And', filters: [] } })
</script>

<style scoped lang="scss">
  .query-group { border-left: 2px solid var(--el-border-color); padding: 10px 0 0 12px; }
  .group-toolbar, .condition-row { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }
  .query-node { margin-top: 10px; }
  .field-select { width: 150px; } .operator-select { width: 140px; } .value-input { flex: 1; min-width: 180px; }
</style>
