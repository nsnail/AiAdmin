<template>
    <ElDrawer v-model="visible" destroy-on-close direction="rtl" size="min(680px, 100%)" title="高级查询">
        <div class="drawer-content">
            <div class="saved-row">
                <ElSelect v-model="selectedSaved" @change="loadSaved" class="saved-select" clearable filterable placeholder="已保存查询">
                    <ElOption
                ...
                <ElTooltip content="保存当前查询"
                    ><ElButton @click="saveQuery" circle><ArtSvgIcon icon="ri:save-line" /></ElButton
                ></ElTooltip>
                <ElTooltip content="删除已保存查询"
                    ><ElButton :disabled="!selectedSaved || (selectedQueryIsGlobal && !isSuperAdmin)" @click="deleteSaved" circle
        ...
        <template #footer><ElButton @click="visible = false">取消</ElButton><ElButton @click="apply" type="primary">应用查询</ElButton></template>
    </ElDrawer>
</template>
...
const selectedQueryIsGlobal = computed(() => savedQueries.value.find((item) => String(item.id) === selectedSaved.value)?.isGlobal === true)
const jsonText = ref('{}')
const jsonError = ref('')
...
const saveQuery = async () => {
    ...
    savedQueries.value = [saved, ...savedQueries.value.filter((item) => String(item.id) !== String(saved.id) && item.name !== saved.name)]
