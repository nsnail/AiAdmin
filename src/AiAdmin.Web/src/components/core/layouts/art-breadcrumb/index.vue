<!-- 面包屑导航 -->
<template>
    <nav aria-label="breadcrumb" class="ml-2.5 max-lg:!hidden">
        <ul class="flex-c h-full">
            <li v-for="(item, index) in breadcrumbItems" :key="item.path" class="box-border flex-c h-7 text-sm leading-7">
                <div
                    ...
                    <span class="block max-w-46 overflow-hidden text-ellipsis whitespace-nowrap px-1.5 text-sm text-g-600 dark:text-g-800">{{
                        formatMenuTitle(item.meta?.title as string)
                    }}</span>
                </div>
                <div v-if="!isLastItem(index) && item.meta?.title" aria-hidden="true" class="mx-1 text-sm not-italic text-g-500">/</div>
            </li>
        </ul>
    </nav>
</template>
