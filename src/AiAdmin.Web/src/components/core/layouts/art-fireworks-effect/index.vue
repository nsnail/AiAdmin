<!-- 烟花效果 | 礼花效果 -->
<template>
    <canvas class="fixed top-0 left-0 z-[9999] w-full h-full pointer-events-none" ref="canvasRef"></canvas>
</template>
...
const CONFIG: FireworkConfig = {
    ...
    SHAPES: ['rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'circle', 'triangle', 'oval'],
} as const
...
class FireworkSystem {
    ...
    createFirework(imageUrl?: string): void {
        ...
        for (let i = 0; i < CONFIG.PARTICLES_PER_BURST; i++) {
            ...
            particle.rotationSpeed = (Math.random() * CONFIG.ROTATION.RANDOM_SPEED + CONFIG.ROTATION.BASE_SPEED) * (Math.random() > 0.5 ? 1 : -1) // 随机旋转方向
    ...
    private isOutOfBounds(particle: Firework): boolean {
        ...
        return particle.x < -margin || particle.x > this.canvasWidth + margin || particle.y < -margin || particle.y > this.canvasHeight + margin
    }
...
const handleKeyPress = (event: KeyboardEvent): void => {
    const isFireworkShortcut =
        (event.ctrlKey && event.shiftKey && event.key.toLowerCase() === 'p') || (event.metaKey && event.shiftKey && event.key.toLowerCase() === 'p')
