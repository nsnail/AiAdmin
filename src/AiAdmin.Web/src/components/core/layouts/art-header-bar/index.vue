    ...
    <div
        ...
        <div
            ...
            <div class="flex-c flex-1 min-w-0 leading-15" style="display: flex">
                <!-- 系统信息  -->
                <div v-if="isTopMenu" @click="toHome" class="flex-c c-p">
                    <ArtLogo class="pl-4.5" />
                    <p v-if="width >= 1400" class="my-0 mx-2 ml-2 text-lg">{{ AppConfig.systemInfo.name }}</p>
                </div>

                <ArtLogo @click="toHome" class="!hidden pl-3.5 overflow-hidden align-[-0.15em] fill-current" />

                <!-- 菜单按钮 -->
                <ArtIconButton v-if="isLeftMenu && shouldShowMenuButton" @click="visibleMenu" class="ml-3 max-sm:ml-[7px]" icon="ri:menu-2-fill" />
                ...
                <ArtFastEnter v-if="shouldShowFastEnter && width >= headerBarFastEnterMinWidth">
                    <ArtIconButton class="ml-3" icon="ri:function-line" />
                </ArtFastEnter>
            ...
            <div class="flex-c gap-2.5">
                ...
                <div
                    ...
                    <div class="flex-c">
                        <ArtSvgIcon class="text-sm text-g-500" icon="ri:search-line" />
                    ...
                    <div class="flex-c h-5 px-1.5 text-g-500/80 border border-g-400 rounded">
                        <ArtSvgIcon v-if="isWindows" class="text-sm" icon="vaadin:ctrl-a" />
                        <ArtSvgIcon v-else class="text-xs" icon="ri:command-fill" />
                ...
                <ElDropdown v-if="shouldShowLanguage" @command="changeLanguage" popper-class="langDropDownStyle">
                    <ArtIconButton class="language-btn text-[19px]" icon="ri:translate-2" />
                    <template #dropdown>
                        <ElDropdownMenu>
                            <div v-for="item in languageOptions" :key="item.value" class="lang-btn-item">
                                <ElDropdownItem :class="{ 'is-selected': locale === item.value }" :command="item.value">
                ...
                <!-- 聊天按钮 -->
                <ArtIconButton v-if="shouldShowChat" @click="openChat" class="chat-button relative" icon="ri:message-3-line">
                    <div class="breathing-dot absolute top-2 right-2 size-1.5 !bg-success rounded-full"></div>
                </ArtIconButton>

                <!-- 设置按钮 -->
                <div v-if="shouldShowSettings">
                    <ElPopover :offset="0" :visible="showSettingGuide" :width="190" placement="bottom-start">
                        <template #reference>
                            <div class="flex-cc">
                                <ArtIconButton @click="openSetting" class="setting-btn" icon="ri:settings-line" />
                            </div>
                        </template>
                        <template #default>
                            <p>
                ...
                <!-- 主题切换按钮 -->
                <ArtIconButton v-if="shouldShowThemeToggle" :icon="isDark ? 'ri:sun-fill' : 'ri:moon-line'" @click="themeAnimation" />
