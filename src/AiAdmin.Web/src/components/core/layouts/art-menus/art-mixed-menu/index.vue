...
<template>
    <div class="relative box-border flex-c w-full overflow-hidden">
        <!-- 左侧滚动按钮 -->
        <div v-show="showLeftArrow" @click="scroll('left')" class="button-arrow">
            <ElIcon>
                <ArrowLeft />
            </ElIcon>
        </div>
        ...
        <!-- 滚动容器 -->
        <ElScrollbar :horizontal="true" @scroll="handleScroll" @wheel="handleWheel" ref="scrollbarRef" wrap-class="scrollbar-wrapper">
            <div class="box-border flex-c flex-shrink-0 flex-nowrap h-15 whitespace-nowrap">
                <template v-for="item in processedMenuList" :key="item.meta.title">
                    <div
                        ...
                        <ArtSvgIcon :class="item.isActive && '!text-theme'" :icon="item.meta.icon" class="text-lg text-g-700 dark:text-g-800 mr-1" />
                        <span :class="item.isActive && '!text-theme'" class="text-md text-g-700 dark:text-g-800">
                            {{ item.formattedTitle }}
                        </span>
        ...
        <!-- 右侧滚动按钮 -->
        <div v-show="showRightArrow" @click="scroll('right')" class="button-arrow right-2">
            <ElIcon>
                <ArrowRight />
            </ElIcon>
        </div>
...
const scroll = (direction: ScrollDirection): void => {
    ...
    const targetScroll =
        direction === 'left' ? currentScroll - SCROLL_CONFIG.BUTTON_SCROLL_DISTANCE : currentScroll + SCROLL_CONFIG.BUTTON_SCROLL_DISTANCE
