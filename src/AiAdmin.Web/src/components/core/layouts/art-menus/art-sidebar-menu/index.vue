<!-- 左侧菜单 或 双列菜单 -->
<template>
    <div v-if="showLeftMenu || isDualMenu" :class="{ 'no-border': menuList.length === 0 }" class="layout-sidebar">
        <!-- 双列菜单（左侧） -->
        <div v-if="isDualMenu" :style="{ width: dualMenuShowText ? '80px' : '64px', background: getMenuTheme.background }" class="dual-menu-left">
            <ArtLogo @click="navigateToHome" class="logo" />
            ...
            <ArtIconButton @click="toggleDualMenuMode" class="switch-btn size-10" icon="ri:arrow-left-right-fill" />
        </div>
        ...
        <div
            ...
            <ElScrollbar :style="scrollbarStyle">
                <ElMenu
                    ...
                    <SidebarSubmenu :isMobile="isMobileMode" :list="menuList" :theme="getMenuTheme" @close="handleMenuClose" />
                </ElMenu>
            </ElScrollbar>

            <!-- 双列菜单右侧折叠按钮 -->
            <div v-if="isDualMenu" @click="toggleMenuVisibility" class="dual-menu-collapse-btn">
                <ArtSvgIcon :icon="menuOpen ? 'ri:arrow-left-wide-fill' : 'ri:arrow-right-wide-fill'" class="text-g-500/70" />
            </div>
