    ...
    <div class="layout-lock-screen">
        ...
        <div v-if="!isLock">
            <ElDialog v-model="visible" :show-close="false" :width="370" @open="handleDialogOpen">
                <div class="flex-c flex-col">
                    ...
                    <ElForm :model="formData" :rules="rules" @submit.prevent="handleLock" class="w-[90%]" ref="formRef">
                        <ElFormItem prop="password">
                            <ElInput
                                ...
                                type="password">
                                <template #suffix>
                                    <ElIcon @click="handleLock" class="c-p">
                                        <Lock />
                                    </ElIcon>
                                </template>
                            </ElInput>
                        </ElFormItem>
                        <ElButton v-ripple @click="handleLock" class="w-full mt-0.5" type="primary">
        ...
        <div v-else class="unlock-content">
            <div class="flex-c flex-col w-80">
                <ElAvatar :size="64" :src="userInfo.avatar" aria-label="用户头像" class="mt-5">
                    <ArtSvgIcon icon="ri:user-line" />
                </ElAvatar>
                <div class="mt-3 mb-3.5 text-base font-medium">
                    {{ userInfo.userName }}
                </div>
                <ElForm :model="unlockForm" :rules="rules" @submit.prevent="handleUnlock" class="w-full !px-2.5" ref="unlockFormRef">
                    <ElFormItem prop="password">
                        <ElInput
                            ...
                            type="password">
                            <template #suffix>
                                <ElIcon @click="handleUnlock" class="c-p">
                                    <Unlock />
                                </ElIcon>
                            </template>
                        </ElInput>
                    ...
                    <ElButton v-ripple @click="handleUnlock" class="w-full mt-2" type="primary">
                        {{ $t('lockScreen.unlock.btnText') }}
                    </ElButton>
                    <div class="w-full text-center">
                        <ElButton @click="toLogin" class="mt-2.5 !text-g-600 hover:!text-theme hover:!bg-transparent" text>
...
const disableDevTools = () => {
    ...
    const checkDevTools = () => {
        ...
        const isDevToolsOpen = window.outerHeight - window.innerHeight > threshold || window.outerWidth - window.innerWidth > threshold
