...
<template>
    <div
        ...
        ref="containerRef">
        <div :style="{ backgroundColor: bgColor }" class="flex-cc absolute left-0 h-full w-9 z-10">
            <ArtSvgIcon class="text-lg" icon="ri:volume-down-line" />
        </div>
        ...
        <div
            ...
            ref="contentRef">
            <!-- 原始内容 -->
            <span class="inline-block" ref="textRef">
                <slot>
                    <span v-html="text"></span>
                </slot>
            </span>
            <!-- 克隆内容用于无缝循环 -->
            <span v-if="shouldClone" :style="cloneSpacing" class="inline-block">
                <slot>
                    <span v-html="text"></span>
                </slot>
            </span>
        ...
        <div v-if="showClose" :style="{ backgroundColor: bgColor }" @click="handleClose" class="flex-cc absolute right-0 h-full w-9 c-p">
            <ArtSvgIcon class="text-lg" icon="ri:close-fill" />
        </div>
    </div>
</template>
