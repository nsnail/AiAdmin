        ...
        <div class="flex-cc gap-1.5 mr-2 max-sm:mr-5">
            <div class="color-picker-expandable relative flex-c max-sm:!hidden">
                <div class="color-dots absolute right-0 rounded-full flex-c gap-2 rounded-5 px-2.5 py-2 pr-9 pl-2.5 opacity-0">
                    <div
                        ...
                        <ArtSvgIcon v-if="color === systemThemeColor" class="text-white" icon="ri:check-fill" />
                    </div>
                </div>
                <div class="btn palette-btn relative z-[2] h-8 w-8 c-p flex-cc tad-300">
                    <ArtSvgIcon class="text-xl text-g-800 transition-colors duration-300" icon="ri:palette-line" />
                </div>
            </div>
            <ElDropdown v-if="shouldShowLanguage" @command="changeLanguage" popper-class="langDropDownStyle">
                <div class="btn language-btn h-8 w-8 c-p flex-cc tad-300">
                    <ArtSvgIcon class="text-[19px] text-g-800 transition-colors duration-300" icon="ri:translate-2" />
                </div>
                <template #dropdown>
                    <ElDropdownMenu>
                        <div v-for="lang in languageOptions" :key="lang.value" class="lang-btn-item">
                            <ElDropdownItem :class="{ 'is-selected': locale === lang.value }" :command="lang.value">
                                <span class="menu-txt">{{ lang.label }}</span>
                                <ArtSvgIcon v-if="locale === lang.value" class="text-base" icon="ri:check-fill" />
                            </ElDropdownItem>
                        </div>
                    </ElDropdownMenu>
                </template>
            </ElDropdown>
            <div v-if="shouldShowThemeToggle" @click="themeAnimation" class="btn theme-btn h-8 w-8 c-p flex-cc tad-300">
