...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(config: UseTableConfig<TApiFn>) {
    ...
    const sortableColumnsFactory = columnsFactory
        ...
        : undefined
    const columnConfig = sortableColumnsFactory ? useTableColumns<TRecord>(sortableColumnsFactory) : null
     ...
     */
    const handleCellQuery = async (condition: { field: string; operator: string; value: unknown }): Promise<void> => {
        const paramsRecord = searchParams as Record<string, unknown>
        const currentFilter = paramsRecord.dynamicFilter
        paramsRecord.dynamicFilter = currentFilter ? { logic: 'And', filters: [currentFilter, condition] } : condition
        await getDataByPage()
    }
