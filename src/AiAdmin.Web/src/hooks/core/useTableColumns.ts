...
export interface DynamicColumnConfig<T = any> {
     ...
     */
    updateColumn: (prop: string | Array<{ prop: string; updates: Partial<ColumnOption<T>> }>, updates?: Partial<ColumnOption<T>>) => void
    /**
...
export function useTableColumns<T = any>(
    ...
    const createColumns = () =>
        columnsFactory().map((column) => ({
            ...column,
            sortable: column.sortable ?? (column.prop && column.prop !== 'operation' && !column.type ? true : undefined),
        }))
    ...
    return {
         ...
         * 更新列（支持单个或批量）
         */
        updateColumn: (prop: string | Array<{ prop: string; updates: Partial<ColumnOption<T>> }>, updates?: Partial<ColumnOption<T>>) => {
            // 批量模式：prop 是数组
