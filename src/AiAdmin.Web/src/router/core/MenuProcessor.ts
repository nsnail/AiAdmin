...
export class MenuProcessor {
            ...
            if (hasPermission) {
                ...
                if (item.children?.length) {
                    ...
                    filteredItem.children = item.children.flatMap((child) => {
                        ...
                        return allowedChildNames.has(child.name) ? [filteredChildren.find((candidate) => candidate.name === child.name)!] : []
                    })
                }
    ...
    private isNavigableRoute(route: AppRouteRecord): boolean {
        return Boolean(
            route.path && route.path !== '/' && !route.meta?.link && route.meta?.isIframe !== true && route.component && route.component !== '',
        )
    }
