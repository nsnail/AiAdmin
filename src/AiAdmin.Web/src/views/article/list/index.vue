    ...
    <div class="page-content !mb-5">
        <ElRow :gutter="10" justify="space-between">
            <ElCol :lg="6" :md="6" :sm="14" :xs="16">
                <ElInput v-model="searchVal" :prefix-icon="Search" @keyup.enter="searchArticle" clearable placeholder="输入文章标题查询" />
            </ElCol>
            ...
            <ElCol :lg="6" :md="6" :sm="10" :xs="6" style="display: flex; justify-content: end">
                <ElButton v-auth="'add'" @click="toAddArticle">新增文章</ElButton>
            </ElCol>
        </ElRow>
            ...
            <div class="grid grid-cols-5 gap-5 max-2xl:grid-cols-4 max-xl:grid-cols-3 max-lg:grid-cols-2 max-sm:grid-cols-1">
                <div
                    ...
                    <div class="relative aspect-[16/9.5]">
                        ...
                        <span class="absolute top-1 right-1 bg-black/50 rounded text-xs px-1 py-0.5 text-white">{{ item.type_name }}</span>
                    </div>
                    <div class="px-2 py-1">
                        <h2 class="text-base text-g-800 font-medium">{{ item.title }}</h2>
                        <div class="flex-b w-full h-6 mt-1">
                            <div class="flex-c text-g-500">
                                <ArtSvgIcon class="mr-1 text-sm" icon="ri:time-line" />
                                <span class="text-sm">{{ useDateFormat(item.create_time, 'YYYY-MM-DD') }}</span>
                                <div class="w-px h-3 bg-g-400 mx-3.5"></div>
                                <ArtSvgIcon class="mr-1 text-sm" icon="ri:eye-line" />
                                <span class="text-sm">{{ item.count }}</span>
                            </div>
                            <ElButton v-auth="'edit'" @click.stop="toEdit(item)" class="opacity-0 group-hover:opacity-100" size="small"
                                >编辑</ElButton
                            >
                        </div>
                    </div>
