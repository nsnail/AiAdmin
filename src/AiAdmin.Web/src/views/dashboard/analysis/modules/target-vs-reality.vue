<template>
    <div class="art-card h-100 p-5 mb-5 max-sm:mb-4">
        ...
        <ArtBarChart :data="revenueData" :showAxisLine="false" :xAxisData="weekDays" barWidth="28%" class="p-5" height="12rem" />

        <div class="px-5 mt-4">
            <div v-for="item in totalItems" :key="item.label" class="flex-c mb-5 last:mb-0">
                <div class="flex-c justify-start w-3/5 text-sm">
                    <div :class="item.iconClass" class="w-10 h-10 mr-3 text-lg rounded-md flex-cc">
                ...
                <div :class="item.valueClass" class="text-lg font-normal">{{ item.value }}</div>
            </div>
        </div>
    </div>
</template>
