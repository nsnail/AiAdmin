...
<template>
    <div class="w-full py-2">
        <!-- 页面头部 -->
        <div class="mb-6">
            <h2 class="m-0 mb-2 text-xl font-medium">{{ $t('menus.examples.permission.pageVisibility') }}</h2>
            <p class="m-0 text-sm leading-[1.6] text-g-700">
                此页面仅对<strong class="font-semibold text-warning">超级管理员</strong>用户可见，演示页面级别的权限控制。
        ...
        <div class="mb-6 last:mb-0">
            <ElCard class="art-card-xs">
                ...
                <div>
                    <ElTimeline>
                        <ElTimelineItem size="large" timestamp="前端控制模式" type="primary">
                            <ElCard>
                                <h4 class="m-0 mb-2 text-base font-semibold">基于角色的权限控制</h4>
                                <p class="m-0 mb-2 leading-[1.6] text-g-700">
                                    在前端控制模式下，页面访问权限由路由配置文件中的
                                    <code class="px-1.5 py-0.5 font-mono text-xs text-theme bg-theme/12 rounded">meta.roles</code>
                                    字段定义，前端会根据用户接口所拥有的角色对路由和菜单进行过滤与控制
                                </p>
                                <pre
                                    class="p-4 mt-3 mb-0 overflow-x-auto font-mono text-xs leading-[1.5] bg-g-200 border-full-d rounded-md"><code class="">{
  ...
  meta: {
    ...
    keepAlive: true
                                ...
                                <ul class="pl-5 my-2">
                                    <li class="my-1 leading-[1.5] text-g-700">用户登录后，接口返回用户角色信息（如 R_SUPER、R_ADMIN、R_USER）</li>
                                    <li class="my-1 leading-[1.5] text-g-700">
                                        在
                                        <code class="px-1.5 py-0.5 font-mono text-xs text-theme bg-theme/12 rounded">beforeEach</code>
                                        路由守卫中检查目标路由的
                                        <code class="px-1.5 py-0.5 font-mono text-xs text-theme bg-theme/12 rounded">roles</code>
                                        配置
                                    </li>
                                    <li class="my-1 leading-[1.5] text-g-700">比较用户角色是否包含在允许访问的角色列表中</li>
                                    <li class="my-1 leading-[1.5] text-g-700">权限不足时跳转到 403 页面</li>
                                </ul>
                            </ElCard>
                        </ElTimelineItem>

                        <ElTimelineItem size="large" timestamp="后端控制模式" type="warning">
                            <ElCard>
                                <h4 class="m-0 mb-2 text-base font-semibold">基于菜单接口的权限控制</h4>
                                <p class="m-0 mb-2 leading-[1.6] text-g-700">
                                    在后端控制模式下，页面访问权限由后端统一管理，前端通过解析后端接口返回的菜单列表来生成可访问的路由，从而实现权限控制
                                </p>
                                <p class="m-0 mb-2 leading-[1.6] text-g-700">接口地址：src/api/menuApi.ts getMenuList</p>
                                <pre
                                    class="p-4 mt-3 mb-0 overflow-x-auto font-mono text-xs leading-[1.5] bg-g-200 border-full-d rounded-md"><code class="">
{
  "code": 200,
      ...
      "children": [
        {
          ...
          "meta": {
            ...
            "keepAlive": true
                        ...
                        <ElTimelineItem size="large" timestamp="菜单显示控制" type="success">
                            <ElCard>
                                <h4>侧边栏菜单可见性</h4>
                ...
                <div class="practices-content">
                    <ElRow :gutter="24">
                        <ElCol :span="12" class="!mb-5">
                            <div class="flex-c">
                                <div class="size-10 bg-g-200 flex-cc rounded mr-2">
                                    <ElIcon color="#409EFF" size="20"><Lock /></ElIcon>
                                </div>
                                <div>
                                    <h4>多层权限验证</h4>
                                    <p class="text-g-700 text-sm">在前端路由、后端接口、UI组件等多个层面实施权限控制，确保安全性。</p>
                                </div>
                            </div>
                        </ElCol>
                        <ElCol :span="12">
                            <div class="flex-c">
                                <div class="size-10 bg-g-200 flex-cc rounded mr-2">
                                    <ElIcon color="#67C23A" size="20"><User /></ElIcon>
                                </div>
                                <div>
                                    <h4>基于角色的访问控制</h4>
                                    <p class="text-g-700 text-sm">采用RBAC模型，通过角色分配权限，简化权限管理复杂度。</p>
                                </div>
                            </div>
                        </ElCol>
                        <ElCol :span="12">
                            <div class="flex-c">
                                <div class="size-10 bg-g-200 flex-cc rounded mr-2">
                                    <ElIcon color="#E6A23C" size="20"><Key /></ElIcon>
                                </div>
                                <div>
                                    <h4>细粒度权限控制</h4>
                        ...
                        <ElCol :span="12">
                            <div class="flex-c">
                                <div class="size-10 bg-g-200 flex-cc rounded mr-2">
                                    <ElIcon color="#F56C6C" size="20"><View /></ElIcon>
                                </div>
                                <div>
                                    <h4>安全性优先原则</h4>
                                    <p class="text-g-700 text-sm">始终遵循最小权限原则，确保用户只能访问必要的功能和数据。</p>
                                </div>
                            </div>
                        </ElCol>
                    </ElRow>
