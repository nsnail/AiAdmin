<template>
    <div class="page-content mb-5">
        <div class="mb-15 text-center">
            ...
            <p class="m-0 text-base leading-relaxed text-g-700">基于 WebSocketClient 的实时通信演示，支持连接管理、消息收发和状态监控</p>
        </div>

        <!-- 连接状态和统计信息 -->
        <ElRow :gutter="20" class="mb-15">
            <ElCol :md="8" :sm="12" :xs="24">
                <ElCard :body-style="{ padding: '20px' }" class="h-full border-0">
            ...
            <ElCol :md="8" :sm="12" :xs="24">
                <ElCard :body-style="{ padding: '20px' }" class="h-full border-0">
                    <div class="text-center">
                        <ElTag :type="connectionTagType" class="mb-2" size="large">
        ...
        <ElRow :gutter="20" class="mb-15">
            <ElCol :md="12" :xs="24">
                <ElCard class="h-full border-0">
                    ...
                    <ElForm :model="connectForm" class="max-w-md" label-width="100px">
                        <ElFormItem label="服务器地址">
                            <ElInput v-model="connectForm.url" clearable placeholder="ws://localhost:8080/ws" />
                        </ElFormItem>
                        ...
                        <ElFormItem>
                            <ElSpace>
                                <ElButton :disabled="isConnected" :loading="isConnecting" @click="handleConnect" type="primary">
                                    {{ isConnecting ? '连接中...' : '连接' }}
                                </ElButton>
                                <ElButton :disabled="!isConnected" @click="handleDisconnect" type="danger"> 断开连接 </ElButton>
                                <ElButton :disabled="isConnecting" @click="handleReconnect">重连</ElButton>
                            </ElSpace>
                        </ElFormItem>
                    </ElForm>
                </ElCard>
                ...
                <ElCard class="h-full border-0">
                    ...
                    <ElForm :model="messageForm" @submit.prevent="handleSendMessage">
                        ...
                        <ElFormItem label="消息内容">
                            <ElInput v-model="messageForm.content" :rows="4" placeholder="请输入要发送的消息内容" type="textarea" />
                        </ElFormItem>
                        <ElFormItem>
                            <ElSpace>
                                <ElButton :disabled="!isConnected || !messageForm.content" @click="handleSendMessage" type="primary">
                                    发送消息
                                </ElButton>
                ...
                <ElCard class="border-0">
                    <template #header>
                        <div class="flex items-center justify-between">
                            ...
                            <ElButton @click="clearMessages" size="small">清空记录</ElButton>
                        </div>
                    </template>
                    ...
                    <div class="message-container">
                        <div v-for="(message, index) in messageList" :key="index" class="message-item">
                            <div class="message-header">
                                <ElTag :type="message.type === 'received' ? 'success' : 'info'" size="small">
                        ...
                        <ElEmpty v-if="messageList.length === 0" :image-size="100" description="暂无消息记录" />
                    </div>
                </ElCard>
            </ElCol>
        </ElRow>
        ...
        <ElCard class="border-0">
            <template #header>
                <div class="flex items-center justify-between">
                    ...
                    <ElButton @click="clearLogs" size="small">清空日志</ElButton>
                </div>
            </template>

            <div class="log-container">
                <ElAlert v-for="(log, index) in logList" :closable="false" :key="index" :type="log.type" class="!mb-2">
                    <template #title>
                ...
                <ElEmpty v-if="logList.length === 0" :image-size="100" description="暂无日志记录" />
            </div>
        </ElCard>
    </div>
</template>
