    ...
    <div class="flex flex-col gap-4 pb-5">
            ...
            <template #header>
                <div class="flex-wrap gap-3 flex-cb">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <ElTag effect="light" type="success">智能缓存</ElTag>
                        <ElTag effect="light" type="primary">防抖搜索</ElTag>
                        <ElTag effect="light" type="warning">多种刷新</ElTag>
                        <ElTag effect="light" type="info">错误处理</ElTag>
                    </div>
                </div>
            </template>
            <div>
                <p class="m-0 mb-4 leading-[1.6] text-g-700">
                    集成搜索、刷新、全屏、大小控制、列显示隐藏、拖拽排序、表格样式控制、并内置 useTable 组合式函数，提供强大的组合式
                    API，集成数据获取、智能缓存（LRU算法）、 多种刷新策略等核心功能，全面提升表格开发效率。
                </p>

                <!-- 调试面板 -->
                <div v-if="showDebugPanel" class="my-4">
                        ...
                        <ElCollapseItem name="cache" title="缓存统计与演示">
                            <div class="flex flex-col gap-2">
                                ...
                                <div class="flex gap-2 mt-2">
                                    <ElButton @click="handleClearCache" size="small">清空缓存</ElButton>
                                    <ElButton @click="handleCleanExpiredCache" size="small">清理过期缓存</ElButton>
                                    <ElButton @click="handleTestCache" size="small">测试缓存</ElButton>
                                    <ElButton @click="forceRefreshCacheInfo" size="small">刷新缓存信息</ElButton>
                                </div>
                            </div>
                        </ElCollapseItem>
                            ...
                            <div class="flex flex-col gap-2">
                                <div class="max-h-50 overflow-y-auto">
                                    <div v-if="cacheDebugLogs.length === 0" class="p-5 text-center">
                                        <ElEmpty :image-size="60" description="暂无缓存日志" />
                                    </div>
                                ...
                                <div class="flex gap-2 mt-2">
                                    <ElButton @click="cacheDebugLogs = []" size="small">清空日志</ElButton>
                                </div>
                            </div>
                        </ElCollapseItem>
                        <ElCollapseItem name="request" title="请求状态">
                            <div class="flex flex-col gap-2">
                                ...
                                <div class="flex flex-col gap-2">
                                    ...
                                    <ElText class="max-h-50 p-2 overflow-y-auto text-xs bg-g-200 border border-g-400 rounded-md" tag="pre">{{
                                        JSON.stringify(requestParams, null, 2)
                                    }}</ElText>
                                </div>
                                <div class="flex gap-2 mt-2">
                                    <ElButton @click="handleCancelRequest" size="small">取消请求</ElButton>
                                    <ElButton @click="handleClearData" size="small">清空数据</ElButton>
                                </div>
                            </div>
                        </ElCollapseItem>
                    </ElCollapse>
                ...
                <div class="flex flex-wrap gap-4 mt-4">
                    ...
                    <ElText size="small" type="info"> 💡 缓存功能已启用，可通过调试面板查看详细信息 </ElText>
                </div>
            </div>
        </ElCard>
        ...
        <ElCard class="flex-1 art-table-card" style="margin-top: 0">
            ...
            <ArtTableHeader
                ...
                <template #left>
                    <ElSpace wrap>
                        <ElButton v-ripple @click="handleAdd" type="primary">
                            <ElIcon>
                                <Plus />
                            </ElIcon>
                            新增用户
                        ...
                        <ArtExcelExport
                            ...
                            button-text="导出"
                            filename="用户数据" />
                        <ArtExcelImport @import-error="handleImportError" @import-success="handleImportSuccess" style="margin: 0 12px" />

                        <ElButton v-ripple @click="handleClearData" plain> 清空数据 </ElButton>
                        ...
                        <ElDropdown @command="handleColumnCommand" style="margin-left: 10px">
                            ...
                            <template #dropdown>
                                <ElDropdownMenu>
                                    ...
                                    <ElDropdownItem command="batchToggleColumns">批量切换（性别、手机号）</ElDropdownItem>
                                    <ElDropdownItem command="removeColumn">删除列（状态列）</ElDropdownItem>
                                    <ElDropdownItem command="batchRemoveColumns">批量删除（状态、评分）</ElDropdownItem>
                                    <ElDropdownItem command="updateColumn">更新列（手机号）</ElDropdownItem>
                                    <ElDropdownItem command="batchUpdateColumns">批量更新（性别、手机号）</ElDropdownItem>
            ...
            <ArtTable
                ...
                <template #avatar="{ row }">
                    <div class="flex gap-3 user-info">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="m-0 overflow-hidden font-medium text-ellipsis whitespace-nowrap">{{ row.userName }}</p>
                            <p class="m-0 mt-1 overflow-hidden text-xs text-g-700 text-ellipsis whitespace-nowrap">{{ row.userEmail }}</p>
                        </div>
                    </div>
                </template>
                ...
                <template #operation="{ row }">
                    <div class="flex">
                        <ArtButtonTable :row="row" @click="handleView(row)" type="view" />
                        <ArtButtonTable :row="row" @click="handleAdd()" type="add" />
                        <ArtButtonTable :row="row" @click="handleEdit(row)" type="edit" />
                        <ArtButtonTable :row="row" @click="handleDelete(row)" type="delete" />
                    </div>
                </template>

                <!-- 自定义手机号表头 -->
                <template #userPhone-header="{ column }">
                    <ElPopover :width="200" placement="bottom" trigger="hover">
                        <template #reference>
                        ...
                        <ElInput v-model="phoneSearch" @input="handlePhoneSearch" placeholder="搜索手机号" size="small">
                            <template #prefix>
                                <ElIcon>
                                    <Search />
                                </ElIcon>
            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="p-4 bg-g-200 border-full-d rounded-lg">
                    ...
                    <div class="flex flex-wrap gap-2 mb-3 last:mb-0">
                        <ElButton :type="eventDemoEnabled ? 'success' : 'primary'" @click="toggleEventDemo">
                            {{ eventDemoEnabled ? '关闭' : '开启' }}事件监听
                        </ElButton>
                        <ElButton v-if="eventDemoEnabled" @click="clearEventLogs">清空日志</ElButton>
                    </div>
                    <div v-if="eventDemoEnabled && eventLogs.length > 0" class="p-3 mt-3 bg-g-200 border border-g-400 rounded-md">
                ...
                <div class="p-4 bg-g-200 border-full-d rounded-lg">
                    ...
                    <div class="flex flex-wrap gap-2 mb-3 last:mb-0">
                        <ElSwitch v-model="tableConfig.fixedHeight" active-text="固定高度 (500px)" class="ml-2" inactive-text="自适应高度" />
                    </div>
                </div>
        ...
        <ElCard class="art-card-xs">
            ...
            <div class="flex flex-wrap gap-2 max-md:flex-col">
                ...
                <ElButton v-ripple @click="refreshSoft">
                    <ElIcon>
                        <Refresh />
                    </ElIcon>
                    软刷新
                </ElButton>
                <ElButton v-ripple @click="refreshCreate">
                    <ElIcon>
                        <Plus />
                    </ElIcon>
                    新增后刷新
                </ElButton>
                <ElButton v-ripple @click="refreshUpdate">
                    <ElIcon>
                        <Edit />
                    </ElIcon>
                    编辑后刷新
                </ElButton>
                <ElButton v-ripple @click="refreshRemove">
                    <ElIcon>
                        <Delete />
                    </ElIcon>
                    删除后刷新
