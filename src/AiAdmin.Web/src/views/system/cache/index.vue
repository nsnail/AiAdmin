    ...
    <ElCard class="server-card">
      <template #header
        ><div class="flex items-center justify-between"
          ...
          ><ElButton text circle :title="t('redisCache.actions.refresh')" @click="loadAll"
            ><ArtSvgIcon icon="ri:refresh-line" /></ElButton></div
      ></template>
      <ElDescriptions v-loading="serverLoading" :column="4" border>
        <ElDescriptionsItem :label="t('redisCache.server.endpoint')">{{
          serverInfo?.endpoint || '-'
        }}</ElDescriptionsItem>
        <ElDescriptionsItem :label="t('redisCache.server.version')">{{
          serverInfo?.version || '-'
        }}</ElDescriptionsItem>
        <ElDescriptionsItem :label="t('redisCache.server.mode')">{{
          serverInfo?.mode || '-'
        }}</ElDescriptionsItem>
        <ElDescriptionsItem :label="t('redisCache.server.clients')">{{
          serverInfo?.connectedClients ?? '-'
        }}</ElDescriptionsItem>
        <ElDescriptionsItem :label="t('redisCache.server.usedMemory')">{{
          serverInfo?.usedMemory || '-'
        }}</ElDescriptionsItem>
        <ElDescriptionsItem :label="t('redisCache.server.maxMemory')">{{
          serverInfo?.maxMemory || '-'
        }}</ElDescriptionsItem>
        <ElDescriptionsItem :label="t('redisCache.server.databaseSize')">{{
          serverInfo?.databaseSize ?? '-'
        }}</ElDescriptionsItem>
        <ElDescriptionsItem :label="t('redisCache.server.cpuUsage')">{{
          formatPercent(serverInfo?.cpuUsagePercent)
        }}</ElDescriptionsItem>
        <ElDescriptionsItem :label="t('redisCache.server.uptime')">{{
          formatUptime(serverInfo?.uptimeSeconds)
        }}</ElDescriptionsItem>
        <ElDescriptionsItem :label="t('redisCache.server.hitRate')">{{
          formatPercent(serverInfo?.cacheHitRatePercent)
        }}</ElDescriptionsItem>
      </ElDescriptions>
    </ElCard>
      ...
      <template #header
        ><div class="flex items-center justify-between gap-3"
          ><ElInput
            v-model="pattern"
            clearable
            :placeholder="t('redisCache.filters.pattern')"
            @keyup.enter="loadKeys"
            ><template #prefix><ArtSvgIcon icon="ri:search-line" /></template></ElInput
          ><div class="flex shrink-0 gap-2"
            ><ElButton @click="openDialog()"
              ><ArtSvgIcon icon="ri:add-line" class="mr-1" />{{
                t('redisCache.actions.add')
              }}</ElButton
            ><ElButton text circle :title="t('redisCache.actions.refresh')" @click="loadKeys"
              ><ArtSvgIcon icon="ri:refresh-line" /></ElButton></div></div
      ></template>
      <ArtTable :loading="loading" :data="keys" height="calc(100vh - 330px)">
        <ElTableColumn
          prop="key"
          :label="t('redisCache.fields.key')"
          min-width="300"
        ...
        <ElTableColumn :label="t('redisCache.fields.ttl')" width="160"
          ><template #default="{ row }">{{
            formatTtl(row.timeToLiveMilliseconds)
          }}</template></ElTableColumn
        >
        <ElTableColumn
          class-name="cache-actions-column"
          :label="t('redisCache.fields.actions')"
          width="210"
          fixed="right"
          ><template #default="{ row }"
            ><ArtButtonTable
              type="view"
              :title="t('redisCache.actions.view')"
              @click="openDialog(row.key, true)" /><ArtButtonTable
              v-if="row.type === 'String'"
              type="edit"
              :title="t('redisCache.actions.edit')"
              @click="openDialog(row.key)" /><ArtButtonTable
              type="delete"
              :title="t('redisCache.actions.delete')"
              @click="deleteKey(row.key)" /></template
        ></ElTableColumn>
      </ArtTable>
    </ElCard>
    <ElDialog
      ...
      :title="
        dialogReadonly
          ? t('redisCache.dialog.view')
          : form.key
            ? t('redisCache.dialog.edit')
            : t('redisCache.dialog.add')
      "
      width="680px"
      destroy-on-close
    >
      <ElForm label-width="110px"
        ><ElFormItem :label="t('redisCache.fields.key')" required
          ><ElInput
            v-model="form.key"
            :disabled="dialogReadonly || Boolean(editingKey)" /></ElFormItem
        ><ElFormItem :label="t('redisCache.fields.type')"
          ><ElInput :model-value="valueType" disabled /></ElFormItem
        ><ElFormItem :label="t('redisCache.fields.value')" required
          ><ElInput
            v-model="form.value"
            type="textarea"
            :rows="12"
            :disabled="dialogReadonly" /></ElFormItem
      ...
      <template #footer
        ...
        ><ElButton v-if="!dialogReadonly" type="primary" :loading="saving" @click="save">{{
...
<script setup lang="ts">
  ...
  const openDialog = async (key = '', readonly = false) => {
    ...
    if (key) {
      ...
      Object.assign(form, {
        ...
        value: result.value,
        expireSeconds:
          result.timeToLiveMilliseconds > 0 ? Math.ceil(result.timeToLiveMilliseconds / 1000) : 0
      })
  ...
  const deleteKey = async (key: string) => {
    await ElMessageBox.confirm(
      t('redisCache.messages.confirmDelete', { key }),
      t('redisCache.actions.delete'),
      { type: 'warning' }
    )
  ...
  const formatTtl = (value: number) =>
    value < 0 ? t('redisCache.fields.persistent') : `${Math.ceil(value / 1000)}s`
  ...
  const formatUptime = (value?: number) =>
    value === undefined
      ? '-'
      : `${Math.floor(value / 86400)}d ${Math.floor((value % 86400) / 3600)}h ${Math.floor((value % 3600) / 60)}m`
  onMounted(loadAll)
</script>
