  ...
  <div class="art-full-height">
    <ElCard class="art-table-card">
      <div class="toolbar">
        ...
        <ElUpload multiple :show-file-list="false" :http-request="handleUpload">
        ...
        <ElButton v-if="selectedFiles.length" :icon="Delete" type="danger" @click="removeSelected"
          >删除选中 ({{ selectedFiles.length }})</ElButton
        >
        <nav class="file-breadcrumb ml-2.5" aria-label="breadcrumb">
          <ul class="flex-c h-7">
            <li
            ...
            >
              ...
              >
                ...
                >
                  <ArtSvgIcon
                    :icon="index === 0 ? 'ri:home-4-line' : 'ri:folder-line'"
                    class="mr-1 shrink-0"
                  />
      ...
      <div v-if="uploadTasks.length" class="upload-progress-list">
        <div v-for="task in uploadTasks" :key="task.id" class="upload-progress-item">
          <ArtSvgIcon icon="ri:file-upload-line" class="shrink-0 text-g-600" />
      ...
      >
        <ElTableColumn type="selection" width="48" :selectable="(row) => !row.isParent" />
        <ElTableColumn
          label="名称"
          min-width="360"
          sortable="custom"
        ...
        >
          <template #default="{ row }">
            <ElButton
              ...
              ><ArtSvgIcon class="file-icon" icon="ri:folder-line" />{{
                trimSlash(row.name)
              }}</ElButton
            >
            <span v-else class="file-name"
              ><ArtSvgIcon class="file-icon" icon="ri:file-line" />{{ row.name }}</span
            >
          </template>
        </ElTableColumn>
        <ElTableColumn prop="size" label="大小" width="120" sortable="custom" align="right"
          ><template #default="{ row }">{{
        ...
        >
        <ElTableColumn label="操作" width="160" fixed="right" align="right">
          <template #default="{ row }">
            <ArtButtonTable
              v-if="!row.isDirectory"
...
<script setup lang="ts">
  ...
  const uploadTasks = ref<
    Array<{ id: number; name: string; percent: number; status: 'uploading' | 'success' | 'error' }>
  >([])
  ...
  const displayFiles = computed(() =>
    currentPath.value
      ? [
          { name: '..', size: 0, lastModified: '', isDirectory: true, isParent: true },
          ...files.value
        ]
      : files.value
  )
  ...
  const handleContextMenuSelect = async (item: MenuItemType) => {
    ...
    else if (item.key === 'copyPath')
      await navigator.clipboard?.writeText(joinPath(currentPath.value, file.name))
  ...
  const removeSelected = async () => {
    try {
      await ElMessageBox.confirm(
        `确定删除选中的 ${selectedFiles.value.length} 项？目录将递归删除其全部内容。`,
        '批量删除确认'
      )
