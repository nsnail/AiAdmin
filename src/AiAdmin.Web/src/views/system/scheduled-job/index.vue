  ...
  <div class="art-full-height">
    ...
    <ElCard class="art-table-card" :style="{ 'margin-top': showSearchBar ? '12px' : '0' }">
      ...
      <ArtTable
        ...
        @sort-change="handleSortChange" @cell-query="handleCellQuery"
      >
...
<script setup lang="ts">
  ...
  import {
    fetchCreateScheduledJob, fetchDeleteScheduledJob, fetchGetScheduledJobs, fetchRunScheduledJob,
    fetchScheduledJobExecutions, fetchUpdateScheduledJob, type SaveScheduledJob, type ScheduledJob,
  ...
  const statusMap: Record<number, { label: string; type: 'info' | 'primary' | 'success' | 'danger' | 'warning' }> = {
    0: { label: '等待执行', type: 'info' }, 1: { label: '执行中', type: 'primary' },
    2: { label: '执行成功', type: 'success' }, 3: { label: '执行失败', type: 'danger' }, 4: { label: '超时', type: 'warning' }
  }
  ...
  const describeCron = (value?: string): string => {
    ...
    if (second === '*' && minute === '*' && hour === '*' && day === '*' && month === '*' && week === '*') return t('cronEditor.description.everySecond')
    if (second === '0' && minute === '*' && hour === '*' && day === '*' && month === '*' && week === '*') return t('cronEditor.description.everyMinute')
    if (second === '0' && minute === '0' && hour === '*' && day === '*' && month === '*' && week === '*') return t('cronEditor.description.hourly')
    const secondMatch = second.match(/^(?:\*|\d+)\/(\d+)$/)
    if (secondMatch && minute === '*' && hour === '*' && day === '*' && month === '*' && week === '*') return t('cronEditor.description.everySeconds', { value: secondMatch[1] })
    const minuteMatch = minute.match(/^(?:\*|\d+)\/(\d+)$/)
    if (second === '0' && minuteMatch && hour === '*' && day === '*' && month === '*' && week === '*') return t('cronEditor.description.everyMinutes', { value: minuteMatch[1] })
    if (/^\d+$/.test(hour) && /^\d+$/.test(minute) && /^\d+$/.test(second) && day === '*' && month === '*' && week === '*') {
      return t('cronEditor.description.daily', { time: [hour, minute, second].map((item) => item.padStart(2, '0')).join(':') })
    }
    return t('cronEditor.description.custom', { value: value.trim() })
  }

  const {
    columns, columnChecks, data, loading, pagination, getData, replaceSearchParams, resetSearchParams,
    handleSizeChange, handleCurrentChange, handleSortChange, handleCellQuery, refreshData,
    ...
    core: {
      ...
      columnsFactory: () => [
        { prop: 'id', queryField: 'Id', queryValueType: 'number', label: 'ID', width: 150, sortable: true,
        ...
        { prop: 'cronExpression', queryField: 'CronExpression', label: 'Cron 表达式', minWidth: 180, sortable: true, useSlot: true },
        { prop: 'requestMethod', queryField: 'RequestMethod', label: '请求方法', width: 110, sortable: true, align: 'center',
          formatter: (row) => h(ElTag, { size: 'small', type: 'info' }, () => row.requestMethod) },
        { prop: 'requestUrl', queryField: 'RequestUrl', label: '请求地址', minWidth: 260, sortable: true, showOverflowTooltip: true },
        { prop: 'timeoutSeconds', queryField: 'TimeoutSeconds', queryValueType: 'number', label: '超时（秒）', width: 120, sortable: true, align: 'center' },
        { prop: 'isEnabled', queryField: 'IsEnabled', queryValueType: 'boolean', label: '是否启用', width: 110, sortable: true, align: 'center',
          formatter: (row) => h(ElTag, { size: 'small', type: row.isEnabled ? 'success' : 'info' }, () => row.isEnabled ? '启用' : '禁用') },
        { prop: 'status', queryField: 'Status', queryValueType: 'number', label: '执行状态', width: 120, sortable: true, align: 'center',
          formatter: (row) => { const status = statusMap[row.status] || { label: '未知', type: 'info' as const }; return h(ElTag, { size: 'small', type: status.type }, () => status.label) } },
        { prop: 'lastTriggeredAt', queryField: 'LastTriggeredAt', queryValueType: 'date', label: '最近触发时间', width: 180, sortable: true,
          formatter: (row) => formatTime(row.lastTriggeredAt) },
        { prop: 'lastFinishedAt', queryField: 'LastFinishedAt', queryValueType: 'date', label: '最近完成时间', width: 180, sortable: true,
          formatter: (row) => formatTime(row.lastFinishedAt) },
        { prop: 'lastError', queryField: 'LastError', label: '最近错误', minWidth: 180, sortable: true, showOverflowTooltip: true,
          formatter: (row) => row.lastError || '-' },
        { prop: 'createdAt', queryField: 'CreatedAt', queryValueType: 'date', label: '创建时间', width: 180, sortable: true,
          formatter: (row) => formatTime(row.createdAt) },
        { prop: 'operation', queryField: false, label: '操作', width: 70, fixed: 'right',
          formatter: (row) => h(ArtButtonMore, { list: [
            { key: 'run', label: '立即执行', icon: 'ri:play-circle-line', disabled: row.status === 1 },
  ...
  const executionSearchItems = [
    ...
    {
      label: '请求方法', key: 'RequestMethod', type: 'select', props: {
        clearable: true, options: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE'].map((value) => ({ label: value, value }))
      }
    },
    {
      label: '执行状态', key: 'Status', type: 'select', props: {
        clearable: true, options: Object.entries(statusMap).map(([value, item]) => ({ label: item.label, value: Number(value) }))
      }
    },
  ...
  const {
    columns: executionColumns, columnChecks: executionColumnChecks, data: executionData, loading: executionLoading,
    pagination: executionPagination, getData: executionGetData, replaceSearchParams: executionReplaceSearchParams,
    resetSearchParams: executionResetSearchParams, handleSizeChange: executionHandleSizeChange,
    handleCurrentChange: executionHandleCurrentChange, handleSortChange: executionHandleSortChange,
    handleCellQuery: executionHandleCellQuery, refreshData: executionRefreshData
  } = executionTable

  const handleSearch = (params: ScheduledJobSearchParams): void => { replaceSearchParams(params); void getData() }
  const openDialog = (job?: ScheduledJob): void => { currentJob.value = job; dialogVisible.value = true }
  const saveJob = async (form: SaveScheduledJob): Promise<void> => {
    if (saving.value) return
    saving.value = true
    try {
      if (currentJob.value) { await fetchUpdateScheduledJob(currentJob.value.id, form); await refreshUpdate() }
      else { await fetchCreateScheduledJob(form); await refreshCreate() }
  ...
  const handleAction = async (item: ButtonMoreItem, job: ScheduledJob): Promise<void> => {
    if (item.key === 'edit') { openDialog(job); return }
    if (item.key === 'run') { await fetchRunScheduledJob(job.id); ElMessage.success('作业已加入执行队列'); await refreshUpdate(); return }
