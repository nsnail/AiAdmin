        ...
        <ElCard v-loading="loading" class="art-table-card">
            <template #header>
                <div class="settings-header">
                    ...
                    <ElButton :disabled="!category" :loading="saving" @click="saveSettings" type="primary">
                        <ArtSvgIcon class="mr-1" icon="ri:save-3-line" />{{ t('systemSettings.save') }}
                    </ElButton>
                </div>
            </template>

            <ElForm v-if="category" class="settings-form" label-width="220px">
                ...
                <ElFormItem :label="t('systemSettings.fields.smtpPort')">
                    <ElInputNumber v-model="smtpPort" :max="65535" :min="1" class="w-full" />
                </ElFormItem>
                ...
                <ElFormItem :label="t('systemSettings.fields.smtpUser')">
                    <ElInput v-model="values['SMTP User']" autocomplete="username" maxlength="100" />
                </ElFormItem>
                <ElFormItem :label="t('systemSettings.fields.smtpPassword')">
                    <ElInput v-model="values['SMTP Password']" autocomplete="new-password" maxlength="100" show-password type="password" />
                </ElFormItem>
...
const loadSettings = async () => {
    ...
    try {
        ...
        items.value.forEach((item) => {
            values[item.label] = item.value === 'true' ? true : item.value === 'false' ? false : item.value
        })
    } finally {
        loading.value = false
    }
